...
interface GameArtSelectorProps {
  onArtSelect: (gameArt: string, featuredPlayer: Player | null, featuredPlayerImageUrl: string | null) => void;
...
const GameArtSelector: React.FC<GameArtSelectorProps> = ({
  onArtSelect, escalacaoData, offPlayer = false
}) => {
    ...
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-600 font-display">
          Faça upload da arte do jogo{!offPlayer && ' e selecione o jogador destaque'}
        </p>
      </div>
                ...
                >
                  ...
                  {playersWithImages.map((player) => (
                    <option key={player.id} value={player.id}>
                      {player.number} - {player.name} ({player.imgUrl?.length} image{player.imgUrl?.length !== 1 ? 'ns' : 'm'})
                    </option>
                  ))}
                </select>
              </div>
