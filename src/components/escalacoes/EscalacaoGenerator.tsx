...
import LayerManager from '../generator/LayerManager'; // Importe o novo componente
...
const EscalacaoGenerator: React.FC<EscalacaoGeneratorProps> = ({ onBack, translations }) => {
  ...
  const [renderOrder, setRenderOrder] = useState(['fundo', 'placar', 'logo', 'jogador', 'titulares', 'reservas', 'tecnico', 'info']);
  ...
  const [escalacaoData, setEscalacaoData] = useState<EscalacaoData>({
    ...
    featuredPlayerImgIndex: null, formation: null, selectedPlayers: {}, reservePlayers: [], coach: 'Arne Slot'
  });
  ...
  const handleNextStep = () => {
    if (currentStep === 1) {
      if (matchSelectorRef.current?.submitManualData && !matchSelectorRef.current.canProceed) {
  ...
  const handleMatchSelect = (matchData: Match) => {
    ...
    const formData: MatchFormData = {
      homeTeam: matchData.teams.home.name, awayTeam: matchData.teams.away.name, competition: formatCompetitionRound(matchData, translations),
  ...
  const handleGameArtSelect = (gameArt: string, featuredPlayer: Player | null, featuredPlayerImageUrl: string | null) => {
    setEscalacaoData(prev => ({ ...prev, gameArt, featuredPlayer, featuredPlayerImageUrl }));
  };
    ...
    for (const key of elementsToDraw) {
      ...
      if (key === 'fundo') {
        try {
          ...
          const imgUrl = baseImages.find(img => img.section === 'escalacao' && img.type === config.imageType)?.url;
          if (!imgUrl) throw new Error('Imagem de fundo para escalação não encontrada');
        ...
        } catch (error) { console.warn('Erro ao carregar logo'); }
      } else if (key === 'placar' && escalacaoData.gameArt) {
        try {
          const placarImg = await loadImage(escalacaoData.gameArt);
          const placarHeight = (config.placarSize * 720) / 1280;
          ctx.drawImage(placarImg, config.placarX, config.placarY, config.placarSize, placarHeight);
        } catch (error) { console.warn('Erro ao carregar arte do placar'); }
      } else if (key === 'jogador' && escalacaoData.featuredPlayerImageUrl) {
        try {
          const jogadorImg = await loadImage(escalacaoData.featuredPlayerImageUrl);
          const jogadorHeight = (config.jogadorSize * 1062) / 666;
          ctx.drawImage(jogadorImg, config.jogadorX, config.jogadorY, config.jogadorSize, jogadorHeight);
        } catch (error) { console.warn('Erro ao carregar jogador destaque'); }
        ...
        escalacaoData.formation.positions.forEach((position) => {
          ...
          if (player) {
            ...
            ctx.textAlign = 'right';
            ctx.fillText(String(player.number), config.playerNumberX, currentY);
            ...
            ctx.textAlign = 'left';
            ctx.fillText(player.name.toUpperCase(), config.playerNameX, currentY);

            currentY += config.lineHeightJogadores;
          }
        });
        clearShadow(ctx); // Desliga a sombra após desenhar os titulares
      } else if (key === 'reservas' && escalacaoData.reservePlayers.length > 0) {
        let currentY = config.startYJogadores + (escalacaoData.formation?.positions.length || 11) * config.lineHeightJogadores + config.reserveBlockOffsetY;
        const reserveNames = escalacaoData.reservePlayers.map(p => p.name).join(', ');
        const maxWidth = config.imageType === 'quadrada' ? 300 : config.imageType === 'vertical' ? 400 : 300;
        ...
        ctx.textAlign = 'right';
        ctx.fillText('BANCO', config.reserveBancoX, (currentY + blockHeight / 2 + 9) + 15);
        ...
        for (const l of lines) {
          ctx.fillText(l.toUpperCase(), config.reserveNamesX, reservasY + lineHeight);
          reservasY += lineHeight;
        }
        clearShadow(ctx); // Desliga a sombra
      } else if (key === 'tecnico' && escalacaoData.coach) {
        let currentY = config.startYJogadores + (escalacaoData.formation?.positions.length || 11) * config.lineHeightJogadores + config.reserveBlockOffsetY + 175;
        ...
        ctx.textAlign = 'left';
        ctx.fillText(escalacaoData.coach.toUpperCase(), config.coachNameX, currentY);
        ...
        const { stadium, date, referee, competitionRound } = escalacaoData.matchData;
        ...
        const centerX = config.imageType === 'horizontal' ? 30 : canvas.width / 2; // Coordenada X central é a mesma para ambas as linhas
  ...
  useEffect(() => {
    redrawAllLayers();
  }, [activeElementKey, activeImageType, escalacaoData, renderOrder, redrawAllLayers]);
  ...
  const downloadEscalacao = () => {
    ...
    if (!ctx || !canvasFundoRef.current || !canvasInteracaoRef.current || !canvasFrenteRef.current) {
      console.error("Download abortado: um ou mais refs de canvas não estão prontos.");
      return;
    }
    ...
    link.download = `escalacao-liverpool-${escalacaoData.matchData?.matchDate || 'custom'}-${activeImageType}.png`;
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-red-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              <Button onClick={onBack} variant="ghost" size="sm" className="mr-4 cursor-pointer font-display-medium">
                <ArrowLeft className="w-4 h-4 mr-2" /> Voltar
              </Button>
              <h1 className="text-xl font-display-bold text-gray-800">Gerador de Escalações</h1>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          {steps.map((step, index) => {
            ...
            return (
              <div key={step.id} className="flex items-center">
                <div className={`flex items-center justify-center w-10 h-10 rounded-full border-2 transition-all ${isCompleted ? 'bg-green-600 border-green-600 text-white cursor-pointer' : isActive ? 'bg-red-600 border-red-600 text-white cursor-pointer' : canAccess ? 'border-gray-300 text-gray-500 hover:border-red-300 cursor-pointer' : 'border-gray-200 text-gray-300 cursor-not-allowed'}`} onClick={() => canAccess && setCurrentStep(step.id)}>
                  {isCompleted ? <Check className="w-5 h-5" /> : <step.icon className="w-5 h-5" />}
                </div>
                <div className="ml-3 hidden md:block">
                  <p className={`text-sm font-display-medium ${isActive ? 'text-red-600' : isCompleted ? 'text-green-600' : 'text-gray-500'}`}>{step.title}</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          {currentStep === 1 && <MatchSelector ref={matchSelectorRef} onMatchSelected={handleMatchSelect} escalacaoData={escalacaoData} onValidationChange={setStep1Valid} translations={translations} />}
          {currentStep === 2 && <GameArtSelector onArtSelect={handleGameArtSelect} escalacaoData={escalacaoData} setEscalacaoData={setEscalacaoData} />}
          {currentStep === 3 && <FormationSelector selectedFormation={escalacaoData.formation} onFormationSelect={handleFormationSelect} />}
          {currentStep === 4 && escalacaoData.formation && (
            <div className="space-y-6">
              <PlayerSelector isOfficialLineUp={officialLineUp} formation={escalacaoData.formation} selectedPlayers={escalacaoData.selectedPlayers} onPlayersChange={handlePlayersChange} reservePlayers={escalacaoData.reservePlayers} onReservePlayersChange={handleReservePlayersChange} maxReserves={11} onOfficialStatusChange={setOfficialLineUp} />
              <div className="bg-gray-50 rounded-lg p-4">
                <label className="block text-sm font-display-medium text-gray-700 mb-2">Técnico</label>
                <input type="text" value={escalacaoData.coach} onChange={(e) => setEscalacaoData(prev => ({ ...prev, coach: e.target.value }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent font-display" placeholder="Nome do técnico" />
              </div>
            </div>
          )}
          {currentStep === 5 && (
            <div className="space-y-6">
              <div className="text-center">
                <h3 className="text-xl font-display-bold text-gray-800 mb-2">Gerar Escalação Final</h3>
                <p className="text-gray-600 font-display">Ajuste os elementos e a ordem das camadas antes de gerar.</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-1 space-y-4">
                  ...
                  {activeElementKey && <PositionController elementName={activeElementKey} onMove={(axis, amount) => setConfigs(prev => { const newConfigs = JSON.parse(JSON.stringify(prev)); const key = `${activeElementKey}${axis.toUpperCase()}` as keyof EscalacaoConfig; if (typeof newConfigs[activeImageType][key] === 'number') { newConfigs[activeImageType][key] += amount; } return newConfigs; })} onResize={(amount) => setConfigs(prev => { const newConfigs = JSON.parse(JSON.stringify(prev)); const key = `${activeElementKey}Size` as keyof EscalacaoConfig; if (typeof newConfigs[activeImageType][key] === 'number') { newConfigs[activeImageType][key] += amount; } return newConfigs; })} />}
              ...
              {selectedMatch && <PostTextGenerator postType={'escalacao'} match={selectedMatch} translations={translations} escalacaoOficial={officialLineUp} />}
              <div className="flex justify-center space-x-4">
                <Button onClick={redrawAllLayers} disabled={generating} className="bg-red-600 hover:bg-red-700 text-white cursor-pointer font-display-medium">
                  {generating ? <><Loader className="w-4 h-4 mr-2 animate-spin" /> Gerando...</> : <><ImageIcon className="w-4 h-4 mr-2" /> Gerar Imagem</>}
                </Button>
                {downloadable && <Button onClick={downloadEscalacao} disabled={generating} variant="outline" className="cursor-pointer font-display-medium"><Download className="w-4 h-4 mr-2" /> Download PNG</Button>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className={`flex ${currentStep === 1 ? 'justify-end' : 'justify-between'} mt-6`}>
          {currentStep !== 1 && <Button onClick={() => setCurrentStep(Math.max(1, currentStep - 1))} disabled={currentStep === 1} variant="outline" className="cursor-pointer font-display-medium"><ArrowLeft className="w-4 h-4 mr-2" /> Anterior</Button>}
          {currentStep !== 5 && <Button onClick={handleNextStep} disabled={currentStep === 5 || !canAdvanceToStep(currentStep + 1)} className="bg-red-600 hover:bg-red-700 text-white cursor-pointer font-display-medium">Próximo <ArrowRight className="w-4 h-4 ml-2" /></Button>}
        </div>
      </div>
    </div>
  );
