...
import { ArrowLeft, ArrowRight, Download, Loader, Check, Calendar, ImageIcon, Palette, PieChart } from 'lucide-react';
...
const initialImageGeneratorConfigs: Record<'quadrada' | 'vertical' | 'horizontal', ElementConfig> = {
    quadrada: {
        ...
        logoX: 921, logoY: 31, logoSize: 123, placarX: 92, placarY: -75, placarSize: 600,
        jogadorX: 484, jogadorY: 100, jogadorSize: 950, footerX: 198, footerY: 980, footerSize: 1,
        pieChartX: 215, pieChartY: 325, pieChartSize: 355,
        pieChartLegendX: 195, pieChartLegendY: 760, pieChartLegendSize: 1.7500000000000007,
    },
    vertical: {
        canvasWidth: 1080, canvasHeight: 1920, backgroundX: 0, backgroundY: 0, backgroundSize: 1080,
        logoX: 865, logoY: 203, logoSize: 175, placarX: 40, placarY: 117, placarSize: 785,
        jogadorX: 505, jogadorY: 407, jogadorSize: 950, footerX: 190, footerY: 1445, footerSize: 1.4500000000000004,
        pieChartX: 230, pieChartY: 615, pieChartSize: 415,
        pieChartLegendX: 195, pieChartLegendY: 1165, pieChartLegendSize: 2.000000000000001,
    },
    horizontal: {
        ...
        jogadorX: 175, jogadorY: 100, jogadorSize: 950, footerX: 30, footerY: 1005, footerSize: 1,
        pieChartX: 960, pieChartY: 600, pieChartSize: 280,
        pieChartLegendX: 960, pieChartLegendY: 900, pieChartLegendSize: 1,
    }
};
...
const ConfrontoGenerator: React.FC<ConfrontoGeneratorProps> = ({ onBack, translations }) => {
    ...
    const [renderOrder, setRenderOrder] = useState(['background', 'jogador', 'placar', 'grafico', 'legenda', 'logo', 'info']);
    ...
    const [confrontoData, setConfrontoData] = useState<ConfrontoData>({ homeWins: 0, draws: 0, awayWins: 0 });
    ...
    const steps = [
        ...
        { id: 3, title: 'Dados do Confronto', icon: PieChart, description: 'Insira o histórico' },
    ...
    const canAdvanceToStep = (step: number): boolean => {
        switch (step) {
            ...
            case 4: const total = confrontoData.homeWins + confrontoData.draws + confrontoData.awayWins; return total > 0;
            default: return true;
        }
    };
    ...
    const handleConfrontoChange = (e: React.ChangeEvent<HTMLInputElement>, field: keyof ConfrontoData) => {
    ...
    const drawLayer = useCallback(async (
        ...
        const colorHome = generatorData.matchData?.homeTeam === 'Liverpool' ? '#d32f2f' : '#757575'
        const colorDraw = '#1ae9de'
        const colorAway = generatorData.matchData?.awayTeam === 'Liverpool' ? '#d32f2f' : '#757575'
        ...
        for (const key of elementsToDraw) {
            if (key === 'background') {
                try {
                    const imgUrl = baseImages.find(img => img.type === activeImageType && img.section === 'confronto')?.url;
                    if (imgUrl) {
                ...
                if (total > 0) {
                    ...
                    new Chart(chartCanvas, {
                        type: 'pie',
                        data: {
                            labels: [`Vitórias ${generatorData.matchData?.homeTeam || 'Casa'}`, 'Empates', `Vitórias ${generatorData.matchData?.awayTeam || 'Visitante'}`],
                            datasets: [{
                        ...
                        options: { animation: { duration: 0 }, responsive: false, plugins: { legend: { display: false } } }
                    });
                    await new Promise(resolve => setTimeout(resolve, 50));
                    ctx.drawImage(chartCanvas, config.pieChartX, config.pieChartY, config.pieChartSize, config.pieChartSize);
                }
                clearShadow(ctx);
            } else if (key === 'legenda') {
                applyShadow(ctx);
                ...
                if (total > 0) {
                    ...
                    const legendItems = [
                        { label: `Vitórias ${selectedMatch?.teams.home.name}: ${homeWins}`, color: colorHome },
                        { label: `Empates: ${draws}`, color: colorDraw },
                        { label: `Vitórias ${selectedMatch?.teams.away.name}: ${awayWins}`, color: colorAway },
                    ];
                ...
                const { stadium, date, referee, competitionRound } = generatorData.matchData;
    ...
    const redrawAllLayers = useCallback(() => {
        ...
        const behind = activeIndex > -1 ? renderOrder.slice(0, activeIndex) : renderOrder;
    ...
    useEffect(() => { redrawAllLayers(); }, [redrawAllLayers]);
        ...
        <div className="min-h-screen bg-gradient-to-br from-red-50 to-red-100">
            <header className="bg-white shadow-sm border-b border-gray-200">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex justify-between items-center h-16">
                        ...
                        <h1 className="text-xl font-display-bold text-gray-800">Gerador de Confronto</h1>
                    </div>
                </div>
            </header>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    {currentStep === 1 && <MatchSelector ref={matchSelectorRef} onMatchSelected={handleMatchSelect} escalacaoData={{ ...generatorData, formation: null, selectedPlayers: {}, reservePlayers: [], coach: '' }} onValidationChange={setStep1Valid} translations={translations} />}
                    {currentStep === 2 && <GameArtSelector onArtSelect={handleGameArtSelect} escalacaoData={{ ...generatorData, formation: null, selectedPlayers: {}, reservePlayers: [], coach: '' }} setEscalacaoData={(update) => { let newState; if (typeof update === 'function') { const prevAsEscalacaoData: EscalacaoData = { ...generatorData, formation: null, selectedPlayers: {}, reservePlayers: [], coach: '' }; newState = update(prevAsEscalacaoData); } else { newState = update; } setGeneratorData(prev => ({ ...prev, gameArt: newState.gameArt, featuredPlayer: newState.featuredPlayer, featuredPlayerImageUrl: newState.featuredPlayerImageUrl })); setStep2Valid(!!newState.gameArt && !!newState.featuredPlayer); }} />}
                    {currentStep === 3 && (
                        <div className="space-y-4">
                            <h3 className="text-lg font-display-medium text-center">Histórico do Confronto</h3>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">Vitórias {selectedMatch?.teams.home.name}</label>
                                    <input type="text" value={confrontoData.homeWins || ''} onChange={(e) => handleConfrontoChange(e, 'homeWins')} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">Empates</label>
                                    <input type="text" value={confrontoData.draws || ''} onChange={(e) => handleConfrontoChange(e, 'draws')} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700">Vitórias {selectedMatch?.teams.away.name}</label>
                                    <input type="text" value={confrontoData.awayWins || ''} onChange={(e) => handleConfrontoChange(e, 'awayWins')} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-red-500 focus:border-red-500" />
                                </div>
                            </div>
                        </div>
                    )}
                    {currentStep === 4 && (
                        <div className="space-y-6">
                                ...
                                <div className="md:col-span-1 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <select value={activeElementKey ?? ''} onChange={(e) => setActiveElementKey(e.target.value || null)} className="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                            ...
                                            {renderOrder.map(key => <option key={key} value={key} className="capitalize">{key.replace(/([A-Z])/g, ' $1')}</option>)}
                                        </select>
                                    </div>
                            ...
                            {selectedMatch && <PostTextGenerator postType={'confronto'} match={selectedMatch} translations={translations} confrontoData={confrontoData} />}
