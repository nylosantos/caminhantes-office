...
const UserManagement: React.FC = () => {
  ...
  const getLastLoginStatus = (lastLogin: Date | undefined) => {
    ...
    if (diffDays === 0) return { text: 'Hoje', color: 'text-green-600' };
    if (diffDays === 1) return { text: 'Ontem', color: 'text-green-500' };
    if (diffDays <= 7) return { text: `${diffDays} dias`, color: 'text-yellow-600' };
    if (diffDays <= 30) return { text: `${diffDays} dias`, color: 'text-orange-600' };
    return { text: `${diffDays} dias`, color: 'text-red-600' };
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-12">
        ...
        <span className="ml-2 text-gray-600 font-display">Carregando usuários...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-display-extrabold text-gray-800">Gerenciamento de Usuários</h2>
          <p className="text-gray-600 mt-1 font-display">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-display-medium text-gray-600">Total</p>
              <p className="text-2xl font-display-bold text-gray-800">{users.length}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-display-medium text-gray-600">Ativos</p>
              <p className="text-2xl font-display-bold text-gray-800">
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-display-medium text-gray-600">Admins</p>
              <p className="text-2xl font-display-bold text-gray-800">
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              <p className="text-sm font-display-medium text-gray-600">Inativos</p>
              <p className="text-2xl font-display-bold text-gray-800">
                {users.filter(u => !u.active).length}
              </p>
            </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-right text-xs font-display-semibold text-gray-500 uppercase tracking-wider">
                  Ações
                </th>
              </tr>
            </thead>
                  ...
                  <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10">
                          <div className="h-10 w-10 rounded-full bg-gray-300 flex items-center justify-center">
                            <span className="text-sm font-display-semibold text-gray-700">
                              {user.name.charAt(0).toUpperCase()}
                            </span>
                          </div>
                        </div>
                        <div className="ml-4">
                          <div className="text-sm font-display-medium text-gray-900">{user.name}</div>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        ...
                        <span className={`ml-2 inline-flex px-2 py-1 text-xs font-display-semibold rounded-full ${getRoleBadgeColor(user.role)}`}>
                          {user.role}
                        </span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className={`inline-flex px-2 py-1 text-xs font-display-semibold rounded-full ${
                        user.active 
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-display">
                        <div className={`font-display-medium ${lastLoginStatus.color}`}>
                      ...
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        {currentUserData?.role === 'root' && (
                          <select
                            ...
                            className="text-sm border border-gray-300 rounded px-2 py-1 cursor-pointer font-display"
